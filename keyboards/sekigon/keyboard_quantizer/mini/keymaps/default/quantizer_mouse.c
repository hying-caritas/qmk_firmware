...
void process_gesture(uint8_t layer, gesture_id_t gesture_id) {
    switch (gesture_id) {
        case GESTURE_DOWN_RIGHT ... GESTURE_UP_RIGHT: {
            uint16_t keycode = dynamic_config_keymap_keycode_to_keycode(layer, (MATRIX_MSG_ROWS - 1) * 8 + gesture_id - GESTURE_DOWN_RIGHT);
            if (keycode == MATRIX_MSG_ROWS * 8 + gesture_id - GESTURE_DOWN_RIGHT) {
                return;
            }
            tap_code16(keycode);
        } break;
