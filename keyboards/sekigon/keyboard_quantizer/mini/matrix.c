...
void tuh_hid_mount_cb(uint8_t dev_addr, uint8_t instance, uint8_t const *desc_report, uint16_t desc_len) {
    ...
    if (!parse_report_descriptor((dev_addr * 16) + instance, desc_report, desc_len)) {
        return;
    }
...
void tuh_hid_report_received_cb(uint8_t dev_addr, uint8_t instance, uint8_t const *report, uint16_t len) {
    ...
    // If the previous report has not been consumed by core0 yet, drop this
    // one instead of blocking core1's USB task for up to 50 ms.
    if (len > 0 && hid_report_size == 0) {
