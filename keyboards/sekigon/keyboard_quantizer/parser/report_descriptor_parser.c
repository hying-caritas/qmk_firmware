...
static void delete_hid_device(uint8_t interface) {
    for (int idx = 0; idx < LEN(hid_device_collection); idx++) {
        if (hid_device_collection[idx].interface == interface) {
            ...
            memset(&hid_device_collection[idx], 0, sizeof(hid_device_collection[idx]));
        }
    }
}

static bool get_next_item(uint8_t const **buf, uint16_t *const len, hid_report_item_t *const item) {
    ...
    if (item->tag == 0x0F) {
        // Long item: byte0=prefix, byte1=data_size, byte2=long_tag, bytes3..=data.
        if (*len < 2) {
            return false;
        }
        item->size = data[1];
        item->data = 0;
        item->raw  = 0;
        // after the prefix (1 byte) we need data_size + tag + payload
        if (*len - 1 < 2 + item->size) {
            return false;
        }
        *buf += 2 + item->size;
    } else {
    ...
    while (get_next_item(&buf, &remain_len, &item)) {
        ...
        switch (item.prefix & (HID_RI_TYPE_MASK | HID_RI_TAG_MASK)) {
            ...
            case HID_RI_END_COLLECTION(0):
                ...
                if (level == 1) {
                    if (current_collection != NULL && current_collection->report_def_start != NULL) {
            ...
            case HID_RI_INPUT(0):
                ...
                for (int idx = 0; idx < usage_table_idx; idx++) {
                    ...
                    *new_member        = member;
                    ...
                    } else if (current_collection != NULL && current_collection->report_def_start == NULL) {
...
void print_collection(hid_id_collection_t const *collection) {
    dprintf("ID:%u, USAGE_PAGE:%u, USAGE:%u\n", collection->id, collection->usage_page, collection->usage);
