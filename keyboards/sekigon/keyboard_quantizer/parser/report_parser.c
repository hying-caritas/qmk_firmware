...
static int16_t parse_value(hid_report_member_t const *member, uint8_t const *data, uint8_t len, uint16_t *bit_idx) {
    int16_t result = 0;
    ...
    if (member != NULL) {
        for (uint8_t idx = 0; idx < member->global.report_count; idx++) {
            for (uint8_t dst_bit_idx = 0; dst_bit_idx < member->global.report_size; dst_bit_idx++) {
                if (*bit_idx / 8 >= len) {
                    // report data exhausted before the declared field is complete
                    return result;
                }
...
void mouse_report_parser(hid_report_member_t const *member, uint8_t const *data, uint8_t len) {
    ...
    while (member != NULL) {
        ...
        int16_t  val         = parse_value(member, data, len, &bit_idx);
...
void system_report_parser(hid_report_member_t const *member, uint8_t const *data, uint8_t len) {
    ...
    while (member != NULL) {
        result = parse_value(member, data, len, &bit_idx);
